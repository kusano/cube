...
function searchLeaveSlice(scramble, normal, inverse, axis, lastDirection) {
    let old = scramble;
    scramble = [];
    for (let m of old) {
    ...
    function f(depth, maxDepth) {
        if (depth==maxDepth) {
            if (cube.isLeaveSlice()) {
                let moves2 = [...moves];
                if (movesLast!="") {
                    moves2.push(movesLast);
                }
                if (lastDirection=="inverse") {
                    moves2 = reverse(moves2);
                }

                const cube2 = new Cube();
                if (lastDirection=="normal") {
                    for (const move of reverse(inverse)) {
                        cube2.move(move);
                    }
                    ...
                    for (const move of reverse(normal)) {
                        cube2.move(move);
                    }
                    for (const move of reverse(scramble)) {
                        cube2.move(move);
                    }
                ...
                FR.push(cube2.getFR());
                BRSubsetFB.push(cube2.getBRSubsetFB());
                BRSubsetRL.push(cube2.getBRSubsetRL());
                ...
                for (let i=0; i<moves2.length; i++) {
                    moves2[i] = unfixAxis(axis, moves2[i]);
                }
                ...
                if (axis=="U/D") {
                    data.BRSubsetFB = BRSubsetFB;
                    data.BRSubsetRL = BRSubsetRL;
                }
                if (axis=="R/L") {
                    data.BRSubsetFB = BRSubsetFB;
                    data.BRSubsetUD = BRSubsetRL;
                }
                if (axis=="F/B") {
                    data.BRSubsetUD = BRSubsetFB;
                    data.BRSubsetRL = BRSubsetRL;
                }
...
function search(input, niss, axis) {
    ...
    searchLeaveSlice(scramble, normal, inverse, drAxis, lastDirection);
}
