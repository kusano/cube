...
`;
    ...
    frBadEdge() {
        let bad = 0;
        for (let i=9; i<45; i+=9) {
            if (this.C[i+1]!=this.C[i+4]) {
                bad++;
            }
            if (this.C[i+7]!=this.C[i+4]) {
                bad++;
            }
        }
        return bad;
    ...
    isFR() {
        for (let f=9; f<45; f+=9) {
            if (this.C[f]!=this.C[f+6]) {
                return false;
            }
            if (this.C[f+1]!=this.C[f+4]) {
                return false;
            }
            if (this.C[f+7]!=this.C[f+4]) {
                return false;
            }
            if (this.C[f+2]!=this.C[f+8]) {
                return false;
            }
        }
...
const frTable = new Map();
{
    ...
    // FRが完了した状態を0手で初期化。
    frTable.set(cube.extractLeaveSlice(), 0);
    let P = [cube.extractLeaveSlice()];
    ...
    console.log("FR table constructed:", frTable.size);
}

// 軸 axis のFRを探索。
// niss: normal, inverse, before, full
function searchFR(scramble, normalInput, inverseInput, axis, niss) {
    // U/D軸に変換して探索。
    const scrambleFixed = [];
    for (let m of scramble) {
    ...
    // どちらかを選択する。
    // 選択基準はバッドエッジがより少ないほう。
    function selectUD(cube) {
        cube.move("U2");
        const badU = cube.frBadEdge();
        cube.undo();

        cube.move("D2");
        const badD = cube.frBadEdge();
        cube.undo();
    ...
    function f2(depth, maxDepthN, rev) {
        if (depth==maxDepthN) {
            if (cube.isFR()) {
                ...
                if (!found.has(face)) {
                    ...
                    const solution = analyzeSolution(scramble, normalInput, inverseInput, axis, nor, inv);
                    solution["type"] = "fr";
        ...
        let h = frTableMax+1;
        const ls = new Cube2(cube).extractLeaveSlice();
        if (frTable.has(ls)) {
            h = frTable.get(ls);
        }
    ...
    for (let mode of ["normal", "inverse", "both"]) {
        ...
        for (let depth=0; ; depth++) {
            ...
            for (let depthI=0; depthI<=depth; depthI++) {
                ...
                if (!(mode=="normal" && depthI==0 ||
                      mode=="inverse" && depthI==depth ||
                      mode=="both" && 0<depthI && depthI<depth)) {
                    continue;
                }
...
// niss: normal, inverse, htr, before, full
// axis: dr, ud, fb, rl, all
function search(input, niss, axis) {
    // Parse.
    const scramble = [];
    const normal = [];
    const inverse = [];
    ...
    let lastDirection = "";
    {
        ...
        for (let i=0; i<moves.length; i++) {
            if (moves[i][1]!="2") {
                if (moves[i][0]=="U" || moves[i][0]=="D") {
                    drAxis = "U/D";
                }
                if (moves[i][0]=="F" || moves[i][0]=="B") {
                    drAxis = "F/B";
                }
                if (moves[i][0]=="R" || moves[i][0]=="L") {
                    drAxis = "R/L";
                }
            }
        }
    ...
    for (let a of axes) {
        searchFR(scramble, normal, inverse, a, niss);
    }
}

if (standalone) {
    search(`R' U' F D' U R2 D2 L2 D' B2 D2 U' F2 U' L2 F' D' U L' B' F2 D' L2 B R' U' F
...
onmessage = e => {
    const data = e.data;
    search(data.input, data.niss, data.axis);
